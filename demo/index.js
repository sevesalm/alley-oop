...
const config = {
  ...
  HTTP_PORT: process.env.HTTP_PORT || 80, // to avoid need for sudo (otherwise can use 80)
...
function isPrivateIpV4Address(ip) {
  if (!ip.match(/^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)(\.|$)){4}$/))
    return false;
...
function checkConfig() {
  ...
  if (config.DOMAIN_NAME && config.SERVER_NAME && config.SERVER_PASSWORD) {
    ...
    console.log(
      `  $ DOMAIN_NAME=lan.example.com SERVER_NAME=alley-oop.example.com SERVER_PASSWORD=password npm start\n`
    );
  }
}
...
function updateDns(hostname, ip) {
  return fetch(
    `https://${config.SERVER_NAME}/v1/update?hostname=${hostname}&myip=${ip}`
  );
}
  ...
  return {
    ...
    SNICallback: (domain, cb) => {
      getKeyPair(domain).then(
        ...
        (err) =>
          console.log("SNI Error:", err.message, err.config && err.config.url)
      );
    },
  };
}
